...
nlohmann::json GameRoom::buildNextPrompt() const {
    ...
    if (state.currentPhase == GamePhase::ADOPT) {
        nlohmann::json allowed = nlohmann::json::array({"get_adapt_status", "resolve_feedback", "cancel_disruption_effect", "trade"});
        ...
        prompt["requiredParamsByAction"] = {
            {"get_adapt_status", nlohmann::json::array()},
        ...
        if (!state.isAdaptComplete() && state.adaptCursor < static_cast<int>(state.adaptTrack.size())) {
            for (int tile = 0; tile < GameState::NUM_TILE; ++tile) {
                if (isValidTargetForCursor(state.adaptCursor, tile) && state.isAdaptTileAvailable(tile)) {
        ...
        prompt["uiHints"] = {
            {"adapt", {
                ...
                {"ring", state.isAdaptComplete() ? "NONE" : ringLabelForCursor(state.adaptCursor)},
                {"nextToken", (state.isAdaptComplete() || state.adaptCursor >= static_cast<int>(state.adaptTrack.size()))
                    ? "NONE"
                    : tokenEffectToString(state.adaptTrack[state.adaptCursor])},
                {"validTargets", validTargets},
                {"developChoiceRequired", (!state.isAdaptComplete()

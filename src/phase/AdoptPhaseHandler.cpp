...
ActionResult AdoptPhaseHandler::handleGetAdaptStatus(GameState& state) {
    if (!state.initAdaptTrackIfNeeded()) {
        return {ActionStatus::UNKNOWN_ERROR, "Adapt track could not be initialized"};
    }
    ...
    ActionResult result = ActionResult::success("Adapt status ready");
    result.addMessage("adapt_status", payload.dump());
    return result;
}
...
ActionResult AdoptPhaseHandler::handleResolveFeedback(const Action& action, GameState& state) {
    ...
    if (state.isAdaptComplete()) {
        return ActionResult::success("All Adapt feedback tokens were already resolved");
    }
    ...
    if (tileIt == action.params.end() || decisionIt == action.params.end()) {
        return {ActionStatus::INVALID_ACTION, "resolve_feedback requires params: target_tile, decision"};
    }
    ...
    try {
        ...
        return {ActionStatus::INVALID_TARGET, "target_tile must be an integer"};
    }

    if (!isValidTargetForCursor(state.adaptCursor, targetTile)) {
        return {ActionStatus::INVALID_TARGET, "target_tile not valid for current Adapt slot"};
    }

    if (!state.isAdaptTileAvailable(targetTile)) {
        return {ActionStatus::INVALID_TARGET, "target_tile already occupied this Adapt phase"};
    }

    const std::string decision = decisionIt->second;
    const TokenEffect effect = state.adaptTrack[state.adaptCursor];

    ActionResult result(ActionStatus::SUCCESS);
    if (decision == "cancel") {
        if (state.params.getCybernationLevel() < 1) {
            return {ActionStatus::INSUFFICIENT_RESOURCE, "Not enough cybernation to cancel feedback"};
        }
        ...
        for (const auto& msg : effectResult.messages) {
            result.messages.push_back(msg);
        }
    } else {
        return {ActionStatus::INVALID_ACTION, "decision must be 'cancel' or 'allow'"};
    }
    ...
    result.addMessage("adapt_feedback_resolved", payload.dump());

    if (state.isAdaptComplete()) {
        result.message = "Adapt phase complete: all feedback tokens resolved";
        return result;
    }
    result.message = "Feedback token resolved";
    return result;
}
...
ActionResult AdoptPhaseHandler::handleCancelDisruptionEffect(const Action& action, GameState& state) {
    ...
    if (nameIt == action.params.end()) {
        return {ActionStatus::INVALID_ACTION, "cancel_disruption_effect requires disruption_name"};
    }

    const DisruptionCard* card = state.findDisruptionCardByName(nameIt->second);
    if (!card) {
        return {ActionStatus::INVALID_TARGET, "Unknown disruption_name"};
    }

    if (!card->isCancellable()) {
        return {ActionStatus::INVALID_ACTION, "This disruption does not provide a cancel option"};
    }
    ...
    if (timesIt != action.params.end()) {
        try {
            ...
            return {ActionStatus::INVALID_ACTION, "times must be integer"};
        }
    }
    if (times <= 0) {
        return {ActionStatus::INVALID_ACTION, "times must be > 0"};
    }

    if (card->getCancelCosts().empty()) {
        return {ActionStatus::INVALID_ACTION, "This disruption has no cancel cost"};
    }

    for (const auto& cost : card->getCancelCosts()) {
        CyberParameter param;
        if (!mapDisruptionEffectToCyberParameter(cost.first, param)) {
            return {ActionStatus::INVALID_ACTION, "Unsupported cancel cost type on disruption card"};
        }
    ...
    for (const auto& cost : computedCosts) {
        if (!canApplyParamDelta(state.params, cost.first, cost.second)) {
            return {ActionStatus::INSUFFICIENT_RESOURCE, "Insufficient resources for disruption cancel cost"};
        }
    }
    ...
    ActionResult result = ActionResult::success("Disruption effect cancelled by paying configured costs");
    result.addMessage("disruption_effect_cancelled", payload.dump());
    return result;
}
...
ActionResult AdoptPhaseHandler::applyFeedbackEffect(TokenEffect effect, int tilePos, const Action& action, GameState& state) {
    ...
    if (!tile) {
        return {ActionStatus::INVALID_TARGET, "Invalid target tile"};
    }
    ...
    switch (effect) {
        case TokenEffect::TURN_WILD: {
            ...
            tile->setBase(base);
            tile->removeOverlay();
            ActionResult result = ActionResult::success("Applied TURN_WILD");
            result.addMessage("adapt_effect_applied", R"({"effect":"TURN_WILD"})");
            return result;
        }
        case TokenEffect::TURN_WASTE: {
            ...
            tile->setBase(base);
            tile->removeOverlay();
            ActionResult result = ActionResult::success("Applied TURN_WASTE");
            result.addMessage("adapt_effect_applied", R"({"effect":"TURN_WASTE"})");
            return result;
        }
        case TokenEffect::LOSE_COHESION:
            state.params.adjustParam(CyberParameter::COHESION, -2);
            {
                ActionResult result = ActionResult::success("Applied LOSE_COHESION");
                result.addMessage("adapt_effect_applied", R"({"effect":"LOSE_COHESION","delta":-2})");
                return result;
            }
        case TokenEffect::SOLVE_DISRUPTION:
            // Keep disruption handling minimal for now, as requested.
            {
                ActionResult result = ActionResult::success("SOLVE_DISRUPTION placeholder (no-op)");
                result.addMessage("adapt_effect_applied", R"({"effect":"SOLVE_DISRUPTION","placeholder":true})");
                return result;
            }
        case TokenEffect::DEVELOP_STACK:
            ...
            {
                ActionResult result = ActionResult::success("Applied DEVELOP_STACK");
                nlohmann::json payload = {
                    {"effect", "DEVELOP_STACK"},
                    {"overlayType", tile->hasOverlay() ? stackTypeToLabel(tile->getOverlay().getType()) : "NONE"}
                };
                result.addMessage("adapt_effect_applied", payload.dump());
                return result;
            }
        case TokenEffect::TRANSFORM_STACK:
            ...
            {
                ActionResult result = ActionResult::success("Applied TRANSFORM_STACK");
                result.addMessage("adapt_effect_applied", R"({"effect":"TRANSFORM_STACK"})");
                return result;
            }
        case TokenEffect::UNKNOWN:
        default:
            {
                ActionResult result = ActionResult::success("Unknown feedback token ignored");
                result.addMessage("adapt_effect_applied", R"({"effect":"UNKNOWN","ignored":true})");
                return result;
            }
    }
}
...
ActionResult AdoptPhaseHandler::handleTrade(const Action& action, GameState& state) {
    ...
    if (giveParamIt == action.params.end() || recvParamIt == action.params.end()) {
        return {ActionStatus::INVALID_ACTION, "trade requires params: give_param, receive_param"};
    }
    ...
    if (giveAmtIt != action.params.end()) {
        try { giveAmount = std::stoi(giveAmtIt->second); } catch (...) { return {ActionStatus::INVALID_ACTION, "give_amount must be integer"}; }
    }
    if (recvAmtIt != action.params.end()) {
        try { recvAmount = std::stoi(recvAmtIt->second); } catch (...) { return {ActionStatus::INVALID_ACTION, "receive_amount must be integer"}; }
    }
    if (giveAmount <= 0 || recvAmount <= 0) {
        return {ActionStatus::INVALID_ACTION, "trade amounts must be > 0"};
    }
    ...
    if (!parseCyberParameter(giveParamIt->second, giveParam) ||
        ...
        return {ActionStatus::INVALID_ACTION, "Unknown trade parameter"};
    }
    if (giveParam == recvParam) {
        return {ActionStatus::INVALID_ACTION, "give_param and receive_param must differ"};
    }
    ...
    if (giveParam == CyberParameter::COHESION || recvParam == CyberParameter::COHESION) {
        return {ActionStatus::INVALID_ACTION, "Cohesion cannot be used in trade"};
    }
    ...
    if (beforeGive < giveAmount) {
        return {ActionStatus::INSUFFICIENT_RESOURCE, "Not enough resource to trade"};
    }
    ...
    ActionResult result = ActionResult::success("Disruption-cancel trade payment completed");
    result.addMessage("adapt_disruption_trade", payload.dump());
    return result;
}
...
ActionResult AdoptPhaseHandler::handleCommit(const Action& action, GameState& state) {
    ...
    if (!state.isAdaptComplete()) {
        return {ActionStatus::INVALID_ACTION, "Cannot commit: Adapt feedback is not fully resolved"};
    }
    ...
    ActionResult result = ActionResult::success("Adapt commit finalized");
    result.addMessage("adapt_commit", payload.dump());
    return result;
}
